            ...
            try:
                ...
                for index, row in enumerate(ws.iter_rows()):
                    if len(row) > 3:
                        # 1回目のループの場合
                        if index == 0:
                            output_data.append(f"◆◆解析対象 : [{filename}]")
                        else:
                            value = row[3].value
                            # "理解できなかった"または"理解出来なかった"が含まれている場合
                            if "理解できなかった" in value or "理解出来なかった" in value:
                                index1 = value.find("理解できなかった")
                                index2 = value.find("理解出来なかった")
                                index = max(index1, index2)
                                value = value[index:]
                            # "自身の課題"が含まれている場合
                            if "自身の課題" in value:
                                index = value.index("自身の課題")
                                value = value[:index-2]
                            output_data.append(f"◆{row[1].value} : {value}")
